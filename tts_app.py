...
async def process_text_segment(segment, voice, rate, pitch, style=None):
    """
    【新版本】处理单个文本段落，统一使用 SSML 支持风格和参数。
    """
    ...
    # 构建 SSML (语音合成标记语言) 文本
    # 即使没有风格，也使用 SSML 来统一设置语速和音调，这是更健壮的做法
    ssml_text = (
        f'<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" '
        f'xmlns:mstts="http://www.w3.org/2001/mstts" xml:lang="en-US">'  # lang可以随便写，会被voice覆盖
        f'<voice name="{voice}">'
        f'<prosody rate="{rate_str}" pitch="{pitch_str}">'
    )
    
    # 如果指定了风格，就添加 express-as 标签
    if style:
    ...
    # 使用 Communicate 类处理 SSML 文本
    communicate = edge_tts.Communicate()
    segment_audio = b''
    
    # communicate.stream() 会自动识别 ssml_text 是不是 SSML
    async for chunk in communicate.stream(ssml_text):
        if chunk["type"] == "audio":
